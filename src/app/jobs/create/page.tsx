...
export default function CreateJobPage() {
    ...
    useEffect(() => {
        const checkVerification = async () => {
            ...
            const { data: profile } = await supabase.from('profiles').select('is_verified, company').eq('id', user.id).single()
            if (!profile?.is_verified) { router.push('/verify'); return }
            if (profile.company) setFormData((current) => ({ ...current, company: profile.company }))
            setLoading(false)
        }
        checkVerification()
    }, [router])
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        if (!response.ok) { setError(result.error || 'The listing could not be submitted.'); setLoading(false); return }
    ...
    return <main className="rk-product-page"><div className="rk-shell rk-create-layout">
        ...
        <form onSubmit={handleSubmit} className="rk-create-form">
            ...
            <FormSection number="01" title="The opening" description="Start with details a candidate can verify.">
                <div className="rk-form-pair"><Field label="Company" required><input required value={formData.company} onChange={(event) => setFormData({ ...formData, company: event.target.value })} placeholder="Company name" /></Field><Field label="Role title" required><input required value={formData.role_title} onChange={(event) => setFormData({ ...formData, role_title: event.target.value })} placeholder="Software engineer" /></Field></div>
            ...
            <FormSection number="02" title="Referral route" description="Choose how candidate review should be organised.">
                <div className="rk-choice-grid"><button type="button" data-selected={formData.referral_type === 'single'} onClick={() => setFormData({ ...formData, referral_type: 'single' })}><Check size={18} /><strong>Single referral</strong><span>Review requests as they arrive.</span></button><button type="button" data-selected={formData.referral_type === 'pooling'} onClick={() => setFormData({ ...formData, referral_type: 'pooling' })}><Check size={18} /><strong>Candidate pool</strong><span>Collect a set before choosing.</span></button></div>
                {formData.referral_type === 'pooling' && <Field label="Pool size" required><input type="number" min="2" max="50" value={formData.pool_size} onChange={(event) => setFormData({ ...formData, pool_size: Number.parseInt(event.target.value) })} /></Field>}
            </FormSection>
            ...
            <div className="rk-create-actions"><p>Submitting sends this listing to the admin review queue.</p><button type="submit" className="rk-button rk-button-primary">Submit for review</button><button type="button" onClick={() => router.back()} className="rk-button rk-button-secondary">Cancel</button></div>
        </form>
    </div></main>
}
