...
import { FIXED_CANDIDATE_POOL_SIZE } from '@/lib/jobs/job-submission'
...
export default function CreateJobPage() {
    ...
    const [formData, setFormData] = useState({ company: '', role_title: '', department: '', location: '', job_type: 'full_time', experience_level: 'mid', description: '', requirements: '', job_url: '', referral_type: 'single', pool_size: FIXED_CANDIDATE_POOL_SIZE })
    ...
    useEffect(() => {
        const checkVerification = async () => {
            ...
            if (!profile.company) { router.push('/verify'); return }
            setFormData((current) => ({ ...current, company: profile.company }))
            setLoading(false)
        }
        checkVerification()
    }, [router])
    ...
    return <main className="rk-product-page"><div className="rk-shell rk-create-layout">
        ...
        {submitted ? <section className="rk-product-panel rk-submission-success"><ShieldCheck size={30} /><span className="rk-kicker">Submitted for review</span><h2>Your listing is in verifying mode.</h2><p>{submitted.feedback}</p><p>An admin will make the final publishing decision. The listing is not visible to job seekers yet.</p><button type="button" onClick={() => router.push('/dashboard')} className="rk-button rk-button-primary">Continue</button></section> : <form onSubmit={handleSubmit} className="rk-create-form">
            ...
            <FormSection number="01" title="The opening" description="Start with details a candidate can verify.">
                <div className="rk-form-pair"><Field label="Verified company" required><input required readOnly value={formData.company} aria-readonly="true" /></Field><Field label="Job role" required><input required value={formData.role_title} onChange={(event) => setFormData({ ...formData, role_title: event.target.value })} placeholder="Senior Backend Engineer" /></Field></div>
            ...
            <FormSection number="02" title="Referral route" description="Choose how candidate review should be organised.">
                ...
                {formData.referral_type === 'pooling' && <Field label="Pool size" required><input type="number" value={FIXED_CANDIDATE_POOL_SIZE} readOnly aria-readonly="true" /><small>Candidate pools always contain exactly 10 applications.</small></Field>}
            </FormSection>
